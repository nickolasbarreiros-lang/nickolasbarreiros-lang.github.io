...
function normalizarIluminacao(valor) {
    ...
    if (typeof valor === "string") {
        ...
        // Aceita também um objeto salvo acidentalmente como JSON em texto.
...
function criarCardIluminacao(valorIluminacao) {
    const iluminacao = normalizarIluminacao(valorIluminacao);
    ...
    if (iluminacao.texto) {
        return criarInfoCard({
            titulo: "Iluminação",
            icone: "☀️",
            conteudo: iluminacao.texto,
            classeExtra: "card-iluminacao-v2"
        });
    }

    const solDiretoNormalizado = String(iluminacao.solDireto || "")
        .trim()
        .toLowerCase();

    const textoSol = solDiretoNormalizado === "sim"
        ? "Com sol direto"
        : solDiretoNormalizado === "não" || solDiretoNormalizado === "nao"
            ? "Sem sol direto"
            : iluminacao.solDireto || "";

    const iconeSol = solDiretoNormalizado === "sim" ? "☀️" : "🚫";

    const chips = [
        criarChip(
            "🟨",
            iluminacao.sombrite
                ? `${iluminacao.sombrite} de sombrite`
                : ""
        ),
    ...
    return criarInfoCard({
        ...
        icone: "☀️",
        chips,
        descricao: obterTexto(iluminacao.observacoes, ""),
        classeExtra: "card-iluminacao-v2"
    });
}
...
console.info("Orquidário Digital — ficha visual 1.4 carregada");
