// Ficha atualizada no LOTE 16 — PADRÃO EDITORIAL V3.

export const rodrigueziaVenusta = {
    id: "rodriguezia-venusta",
    nome: "Rodriguezia bracteata",
    genero: "Rodriguezia",
    sinonimo: "Rodriguezia venusta",
    tipo: "Espécie botânica",
    dificuldade: "Fácil a moderada",
    caracteristicas: ["Espécie sul-americana", "Flores claras e perfumadas", "Inflorescências pendentes", "Raízes finas"],
    fotos: [
        "imagens/Rodriguezia bracteata/Rodriguezia bracteata1.webp",
        "imagens/Rodriguezia bracteata/Rodriguezia bracteata2.webp",
        "imagens/Rodriguezia bracteata/Rodriguezia bracteata3.webp",
        "imagens/Rodriguezia bracteata/Rodriguezia bracteata4.jpg"
    ],
    descricao: "Orquídea compacta tradicionalmente comercializada como Rodriguezia venusta e atualmente tratada como Rodriguezia bracteata. Forma touceiras elegantes e produz hastes arqueadas ou pendentes com numerosas flores claras, frequentemente marcadas de amarelo e com fragrância perceptível.",
    origem: "América do Sul tropical.",
    regiao: "Brasil e áreas tropicais adjacentes da América do Sul.",
    habitat: "Epífita de florestas tropicais, sobre troncos e ramos com luz filtrada, umidade atmosférica e boa circulação de ar.",
    clima: "Quente a intermediário, úmido e ventilado.",
    climaFloracao: "Floresce a partir de pseudobulbos maduros, bem iluminados e com raízes saudáveis. Noites ligeiramente mais amenas no fim do inverno podem favorecer a emissão das hastes.",
    iluminacao: {
        sombrite: "50% a 70%",
        solDireto: "Somente sol fraco da manhã",
        horario: "Até aproximadamente 08:00, com adaptação",
        observacoes: "Prefere meia-sombra clara. Sombra excessiva reduz a floração; sol forte causa manchas e desidratação nas folhas."
    },
    floracao: "Mais frequente da primavera ao verão.",
    adubacao: "🌿 Orgânica: bokashi em saquinho e em quantidade pequena.\n💧 Foliar: adubo equilibrado em 1/2 da dose, a cada 7–15 dias durante a brotação.\n🧪 Liberação lenta: Osmocote em dose reduzida, pois as raízes são finas.",
    rega: "Mantenha umidade regular durante o crescimento, mas permita leve secagem entre as regas. Plantas montadas podem precisar de água diária no verão; em vasos, evite substrato permanentemente molhado.",
    suporte: "🥇 Placa de madeira, cortiça ou galho.\n🥈 Cachepô pequeno e bem vazado.\n🥉 Vaso pequeno de barro com drenagem ampla.",
    substrato: "🥇 Montada com fina camada de esfagno.\n🥈 Casca de pinus fina a média + carvão + pedra-pomes.\n🥉 Fibra de coco tratada + carvão, sem compactação.",
    errosComuns: "Cultivar em sombra profunda; usar excesso de esfagno; manter a touceira molhada à noite; colocar bokashi diretamente sobre as raízes; usar vaso grande; impedir que as hastes pendam livremente.",
    dica: "No litoral quente, a montagem vertical costuma oferecer o melhor equilíbrio entre umidade e ventilação. Posicione a planta de modo que as hastes possam pender sem obstáculos.",
    adaptacaoRegional: {
        litoralQuente: "Adapta-se bem em meia-sombra clara, com regas matinais, ventilação constante e secagem rápida das raízes.",
        montanhaFrio: "Tolera noites amenas, mas deve ser protegida de frio intenso e de umidade parada durante o inverno."
    },
    mesesFloracao: [9,10,11,12,1,2],
    avaliacoes: { cultivo: 4, floracao: 4, perfume: 4, luminosidade: 3, agua: 4, raridade: 3 }
};
