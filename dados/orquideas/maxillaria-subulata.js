// Ficha V4 refinada — conteúdo antigo e novo comparados, filtrados e agregados; coluna lateral resumida para preservar o layout.

export const maxillariaSubulata = {
    "id": "maxillaria-subulata",
    "nome": "Maxillaria subulata",
    "genero": "Maxillaria",
    "tipo": "Espécie botânica",
    "dificuldade": "Moderada",
    "caracteristicas": [
        "Epífita simpodial",
        "Folhas estreitas e subuladas",
        "Pseudobulbos agrupados",
        "Flores basais cerosas",
        "Raízes finas e sensíveis"
    ],
    "fotos": [
        "imagens/Maxillaria subulata/Maxillaria subulata1.jpg",
        "imagens/Maxillaria subulata/Maxillaria subulata2.jpg",
        "imagens/Maxillaria subulata/Maxillaria subulata3.jpg",
        "imagens/Maxillaria subulata/Maxillaria subulata4.jpg"
    ],
    "descricao": "Espécie epífita simpodial de porte médio, marcada por pseudobulbos ovoides comprimidos e folhas muito estreitas, eretas e subuladas — isto é, em forma de sovela ou agulha. As inflorescências são curtas e surgem na base dos pseudobulbos maduros, sustentando flores solitárias, cerosas, em tons alaranjados, amarelados ou rosa-amarronzados, com contraste vinoso no labelo. Há táxons morfologicamente semelhantes no comércio; a identificação segura deve considerar flor, labelo, coluna, pseudobulbos, folhas e procedência. O cultivo exige equilíbrio: folhas estreitas e pouca reserva não toleram seca severa, enquanto raízes finas perdem-se rapidamente em recipientes grandes e abafados.",
    "origem": "América Tropical.",
    "regiao": "Florestas tropicais úmidas, inclusive áreas de encosta e altitude.",
    "habitat": "Epífita sobre troncos e galhos, sob umidade alta e ventilação constante.",
    "clima": "Quente a intermediário, com boa resposta a noites frescas.",
    "climaFloracao": "A floração depende de pseudobulbos completamente maduros, raízes ativas e luminosidade filtrada brilhante. Não necessita de repouso seco rígido. Após a maturação, aumente discretamente o intervalo entre regas e reduza o excesso de nitrogênio, sem desidratar a planta. Touceiras adultas mantidas intactas produzem maior número de hastes basais. Em locais quentes, a circulação de ar evita superaquecimento e acelera a secagem da base.",
    "iluminacao": {
        "sombrite": "60% a 70%",
        "solDireto": "Somente sol muito brando",
        "horario": "Primeiras horas da manhã",
        "observacoes": "Necessita luz filtrada brilhante. Folhas firmes em verde-médio indicam equilíbrio. Touceiras muito escuras, alongadas ou amolecidas sugerem sombra excessiva. Amarelecimento forte, pontas ressecadas ou manchas claras indicam excesso de sol ou calor."
    },
    "floracao": "Mais comum na primavera e início do verão, aproximadamente de setembro a dezembro. As hastes são curtas e surgem rente à base dos pseudobulbos, podendo ficar parcialmente escondidas. A intensidade varia conforme procedência e maturação da touceira. O perfume é geralmente discreto e pode ser mais perceptível sob calor moderado e umidade.",
    "adubacao": "🌿 Orgânica: pequena quantidade de bokashi em porta-adubo, afastada do rizoma.\n💧 Foliar/mineral: NPK equilibrado em 1/4 a 1/3 da dose a cada 10–15 dias na atividade vegetativa.\n🧪 Liberação lenta: poucos grânulos em recipiente muito drenante e longe das raízes novas.\n🚿 Flushing mensal com água pura para reduzir salinidade. Hidrate as raízes antes de aplicar fertilizante.",
    "rega": "Regue abundantemente na brotação e permita secagem parcial rápida antes da próxima irrigação. As folhas subuladas e os pseudobulbos relativamente pequenos oferecem reserva limitada, portanto evite seca severa. Ao mesmo tempo, não mantenha o fundo do vaso permanentemente molhado. Após a maturação, aumente levemente o intervalo. Pseudobulbos enrugados com substrato molhado e raízes escuras sugerem perda radicular; pseudobulbos enrugados com raízes firmes e substrato seco indicam déficit de água.",
    "suporte": "🥇 Vaso de barro ou plástico muito perfurado, apenas um pouco maior que a touceira.\n🥈 Cachepô de madeira para máxima aeração das raízes finas.\n🥉 Placa de madeira ou cortiça com fina camada de musgo, indicada apenas onde a umidade ambiental supera 65% e há regas frequentes.",
    "substrato": "Casca de pinus média + pedra-pomes + carvão vegetal. Alternativas: casca + brita fina + 10% a 20% de esfagno picado e solto; ou fibra de coco em cubos bem lavada + carvão + componente mineral. O rizoma deve ficar sobre a superfície. Substrato compactado ou em decomposição precisa ser trocado no início do enraizamento.",
    "errosComuns": "❌ Usar vaso grande, fundo ou pouco perfurado.\n❌ Manter substrato compactado ou encharcado.\n❌ Enterrar rizoma e base dos pseudobulbos.\n❌ Dividir a touceira em porções pequenas.\n❌ Cultivar em sombra profunda e produzir folhas fracas.\n❌ Impor seca intensa por longo período.\n❌ Adubar em concentração alta ou sem lavagem mensal.\n❌ Replantar fora da emissão de raízes novas.",
    "dica": "Use vaso pequeno, muito perfurado, e uma mistura que retenha umidade apenas pelo tempo necessário para hidratar a planta. Folhas estreitas não significam que a espécie deva ficar seca; significam que a reposição de água precisa ser frequente, enquanto as raízes exigem renovação constante de oxigênio. Replante apenas quando surgirem novas pontas radiculares, preserve a maior parte da touceira e mantenha o rizoma exposto. Essa combinação reduz o choque, evita mela e acelera a fixação.",
    "adaptacaoRegional": {
        "litoralQuente": "Em Serra/ES, use ventilação contínua, vaso justo, mistura drenante e regas matinais. Monitore o calor acumulado e o tempo real de secagem.",
        "montanhaFrio": "Em Santa Teresa/ES, as noites frescas favorecem a planta. Em semanas frias e chuvosas, aumente o intervalo entre regas sem impor seca total."
    },
    "mesesFloracao": [
        9,
        10,
        11,
        12
    ],
    "avaliacoes": {
        "cultivo": 3,
        "floracao": 3,
        "perfume": 2,
        "luminosidade": 3,
        "agua": 4,
        "raridade": 3
    }
};
