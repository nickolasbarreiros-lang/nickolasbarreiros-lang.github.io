// Ficha científica e de cultivo revisada — Anathallis sclerophylla.
// Nome mantido no Orquidário Digital conforme identificação adotada no catálogo.
// POWO/WFO atualmente tratam o nome como sinônimo de Stelis sclerophylla.

export const anathallisSclerophylla = {
    id: "anathallis-sclerophylla",
    nome: "Anathallis sclerophylla",
    genero: "Anathallis",
    tipo: "Espécie botânica epífita neotropical",
    sinonimo: "Pleurothallis sclerophylla",
    dificuldade: "Fácil",
    origem: "América Tropical.",
    regiao: "Do México e América Central à América do Sul, incluindo o Brasil.",
    habitat: "Epífita de florestas úmidas e nebulosas, cerca de 480–3.100 m.",
    clima: "Intermediário a fresco, úmido e muito ventilado.",
    descricao: "Anathallis sclerophylla é uma pequena orquídea de aparência elegante, marcada por folhas firmes e coriáceas e por hastes florais finas que se elevam muito acima da folhagem. Quando adulta, pode produzir racemos longos com numerosas flores claras abertas ao mesmo tempo, criando um efeito delicado e bastante ornamental. As flores, em tons esbranquiçados a creme, possuem sépalas estreitas e alongadas e podem apresentar fragrância. Apesar do porte discreto, é uma espécie de ampla distribuição e grande amplitude altitudinal, encontrada desde ambientes relativamente quentes até florestas montanas e nebulosas.",
    caracteristicas: [
        "Miniatura a pequeno porte",
        "Haste floral longa",
        "Flores numerosas",
        "Flores perfumadas",
        "Folhas coriáceas",
        "Alta umidade",
        "Pleurothallidinae"
    ],
    fotos: [
        "imagens/Anathallis sclerophylla/Anathallis sclerophylla1.jpg",
        "imagens/Anathallis sclerophylla/Anathallis sclerophylla2.jpg",
        "imagens/Anathallis sclerophylla/Anathallis sclerophylla3.jpg",
        "imagens/Anathallis sclerophylla/Anathallis sclerophylla4.jpg"
    ],
    "selosCultivo": {
        "rega": { "nivel": "frequente" },
        "climaFloracao": { "faixa": "intermediario" }
    },
    avaliacoes: {
        cultivo: 5,
        floracao: 4,
        perfume: 3,
        luminosidade: 2,
        agua: 4,
        raridade: 3
    },
    mesesFloracao: [9, 10, 11, 12, 1, 2, 3, 4, 5],
    climaFloracao: "A espécie possui grande amplitude altitudinal e pode crescer de condições quentes a frias, mas tende a responder melhor quando recebe noites amenas, alta umidade e ventilação constante. No litoral quente, não é necessário tentar reproduzir frio intenso; o mais importante é reduzir o superaquecimento noturno, manter raízes ativas e oferecer boa diferença térmica entre dia e noite quando possível.",
    iluminacao: {
        sombrite: "70% a 80%",
        solDireto: "Evitar; apenas luz solar muito suave e filtrada",
        horario: "Luz difusa durante todo o dia",
        observacoes: "Prefere iluminação baixa a moderada, típica de ambientes florestais úmidos. Em litoral quente, 70% a 80% de sombreamento é um bom ponto de partida. Sombra excessivamente profunda pode reduzir a floração, enquanto sol direto forte aquece rapidamente as folhas coriáceas e desidrata as raízes."
    },
    floracao: "Pode florescer ao longo de uma janela relativamente ampla entre primavera, verão e outono, variando conforme procedência e clima. As inflorescências podem chegar a cerca de 30 cm, ultrapassando bastante as folhas e carregando numerosas flores abertas simultaneamente.",
    adubacao: "🌿 Orgânica: bokashi apenas em quantidade mínima e preferencialmente isolado em sachê ou porta-adubo.\n💧 Foliar/mineral: NPK equilibrado, como 20-20-20, em aproximadamente 1/4 da dose comercial a cada 10 a 15 dias durante crescimento ativo.\n🧪 Liberação lenta: opcional e em quantidade muito pequena, principalmente em cultivo envasado.\n🚿 Lavagem: faça regas periódicas somente com água pura para reduzir o acúmulo de sais nas raízes finas.",
    rega: "Mantenha umidade frequente durante todo o crescimento, evitando que as raízes permaneçam completamente secas por períodos prolongados. Molhe bem e permita drenagem e renovação de ar após cada irrigação. Em montagens, a frequência pode ser alta no calor; em vaso, ajuste sempre pela velocidade real de secagem.",
    suporte: [
        "🥇 Placa de cortiça, madeira ou casca rugosa com pequena reserva de esfagno junto às raízes.",
        "🥈 Cachepô pequeno e muito ventilado.",
        "🥉 Vaso pequeno, raso e bastante perfurado."
    ],
    substrato: [
        "🥇 Esfagno de fibra longa usado muito solto + pedra-pomes fina.",
        "🥈 Casca de pinus fina + pedra-pomes + pequena quantidade de esfagno.",
        "🥉 Chips de coco pequenos + pedra-pomes + carvão vegetal pequeno.",
        "⭐ Alternativa: montagem em cortiça ou madeira com fina camada de esfagno apenas sob as raízes."
    ],
    errosComuns: [
        "Cultivar em ambiente quente e abafado.",
        "Deixar as raízes secarem completamente por vários dias.",
        "Compactar esfagno em torno das raízes.",
        "Manter o substrato permanentemente encharcado.",
        "Expor ao sol direto forte.",
        "Aplicar fertilizante concentrado em raízes finas.",
        "Usar vaso grande e pouco ventilado.",
        "Confundir alta umidade atmosférica com necessidade de água parada no substrato."
    ],
    revisaoCientificaIAR: {
        "nome": "Anathallis sclerophylla",
        "statusTaxonomico": "Nome mantido no catálogo por decisão editorial; POWO adota Stelis sclerophylla.",
        "sinonimosPrincipais": "Verificados prioritariamente em POWO/GBIF/Tropicos; manter apenas os relevantes ao catálogo.",
        "morfologiaDimensoes": "Anathallis sclerophylla é uma pequena orquídea de aparência elegante, marcada por folhas firmes e coriáceas e por hastes florais finas que se elevam muito acima da folhagem. Quando adulta, pode produzir racemos longos com numerosas flores claras abertas ao mesmo tempo, criando um efeito delicado e bastante ornamental. As flores, em tons esbranquiçados a creme, possuem sépalas e",
        "habitatAltitudeClima": "Epífita de florestas úmidas e nebulosas, cerca de 480–3.100 m. | Intermediário a fresco, úmido e muito ventilado.",
        "distribuicaoGeografica": "Do México e América Central à América do Sul, incluindo o Brasil.",
        "cultivoPremiacao": "Cultivo confrontado com habitat/IOSPE; premiação AOS somente considerada quando dado público verificável foi localizado.",
        "confianca": "alta",
        "fontes": [
            "POWO/Kew",
            "WFO/IPNI",
            "GBIF/Tropicos",
            "IOSPE/OrchidSpecies"
        ]
    },
    indiceDificuldadeCultivo: {
        "versao": "IDC 2.0",
        "indice": 9,
        "classificacao": "Fácil",
        "cultivoEstrelas": 5,
        "criterios": {
            "temperatura": {
                "notaDificuldade": 0,
                "peso": 20
            },
            "rega": {
                "notaDificuldade": 0,
                "peso": 15
            },
            "umidade": {
                "notaDificuldade": 0,
                "peso": 10
            },
            "ventilacao": {
                "notaDificuldade": 0,
                "peso": 10
            },
            "luminosidade": {
                "notaDificuldade": 2,
                "peso": 10
            },
            "raizesSubstrato": {
                "notaDificuldade": 2,
                "peso": 10
            },
            "sazonalidade": {
                "notaDificuldade": 1,
                "peso": 5
            },
            "floracao": {
                "notaDificuldade": 0,
                "peso": 20
            }
        }
    },
    indiceAdaptacaoRegional: {
        "versao": "IAR 2.0 — revisão científica",
        "escala": "compatibilidade regional",
        "metodo": "notas atribuídas manualmente após confronto das fontes botânicas e horticulturais; não derivadas do texto da ficha",
        "litoralQuente": {
            "indice": 80,
            "estrelas": 4,
            "fatores": {
                "habitatNatural": {
                    "nota": 4,
                    "peso": 30
                },
                "temperatura": {
                    "nota": 4,
                    "peso": 30
                },
                "aguaUmidade": {
                    "nota": 5,
                    "peso": 5
                },
                "ventilacao": {
                    "nota": 5,
                    "peso": 5
                },
                "luminosidade": {
                    "nota": 2,
                    "peso": 5
                },
                "sazonalidade": {
                    "nota": 4,
                    "peso": 5
                },
                "floracao": {
                    "nota": 4,
                    "peso": 20
                }
            }
        },
        "montanhaFrio": {
            "indice": 97,
            "estrelas": 5,
            "fatores": {
                "habitatNatural": {
                    "nota": 5,
                    "peso": 30
                },
                "temperatura": {
                    "nota": 5,
                    "peso": 30
                },
                "aguaUmidade": {
                    "nota": 5,
                    "peso": 5
                },
                "ventilacao": {
                    "nota": 5,
                    "peso": 5
                },
                "luminosidade": {
                    "nota": 3,
                    "peso": 5
                },
                "sazonalidade": {
                    "nota": 4,
                    "peso": 5
                },
                "floracao": {
                    "nota": 5,
                    "peso": 20
                }
            }
        }
    },
        adaptacaoRegional: {
        litoral: { texto: "A ampla distribuição mostra tolerância térmica considerável, mas no litoral quente é importante manter sombra clara, umidade atmosférica alta e ventilação constante. Noites muito quentes associadas a ar parado são o principal ponto de atenção." },
        montanha: { texto: "Noites frescas, alta umidade e boa circulação de ar se aproximam bastante das condições montanas e de floresta nebulosa registradas para a espécie." }
    },
    dica: "🏆 O segredo é reproduzir uma floresta úmida sem transformar o vaso em um pântano: mantenha as raízes hidratadas, mas com ar circulando constantemente. Uma montagem com pequena quantidade de esfagno ou um vaso pequeno e muito aerado costuma funcionar melhor que recipientes profundos e compactos.",

    };
