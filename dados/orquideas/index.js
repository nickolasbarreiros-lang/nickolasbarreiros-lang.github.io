// Índice central gerado automaticamente a partir de Orquideas_ALBUM.docx.
// Para incluir uma nova planta: crie o arquivo individual e adicione o import e o nome no array abaixo.

import { sophronitisWittigiana } from "./sophronitis-wittigiana.js?v=20260806-1604-auditoria-estrutural";
import { dendrobiumPolysema } from "./dendrobium-polysema.js";
import { bifrenariaVerboonenii } from "./bifrenaria-verboonenii.js";
import { bifrenariaAureofulva } from "./bifrenaria-aureofulva.js";
import { sophronitisCernua } from "./sophronitis-cernua.js";
import { sophronitisCernuaVarLitoranea } from "./sophronitis-cernua-var-litoranea.js";
import { zygolumLouisendorf } from "./zygolum-louisendorf.js";
import { dendrobiumLoddigesii } from "./dendrobium-loddigesii.js";
import { maxillariaSchunkeana } from "./maxillaria-schunkeana.js";
import { mediocalcarDecoratum } from "./mediocalcar-decoratum.js";
import { cattleyaPumpkinFestival } from "./cattleya-pumpkin-festival.js";
import { cattleyaSchilleriana } from "./cattleya-schilleriana.js";
import { bulbophyllumRothschildianum } from "./bulbophyllum-rothschildianum.js";
import { encycliaMineirinha } from "./encyclia-mineirinha.js";
import { arpophyllumGiganteum } from "./arpophyllum-giganteum.js";
import { oncidiumTwinkle } from "./oncidium-twinkle.js";
import { dendrobiumTenellum } from "./dendrobium-tenellum.js";
import { dendrobiumAggregatum } from "./dendrobium-aggregatum.js";
import { dendrobiumMoscatum } from "./dendrobium-moscatum.js";
import { rodrigueziaVenusta } from "./rodriguezia-venusta.js";
import { rodrigueziaLanceolata } from "./rodriguezia-lanceolata.js";
import { oncidiumPumilum } from "./oncidium-pumilum.js";
import { eriaHyacinthoides } from "./eria-hyacinthoides.js";
import { renantheraCoccinea } from "./renanthera-coccinea.js";
import { spathoglottisPlicata } from "./spathoglottis-plicata.js";
import { cyrtopodiumPodophyllum } from "./cyrtopodium-podophyllum.js";
import { aciantheraGlumacea } from "./acianthera-glumacea.js";
import { miltoniaClowesii } from "./miltonia-clowesii.js";
import { gomesaRecurva } from "./gomesa-recurva.js";
import { maxillariaSubulata } from "./maxillaria-subulata.js";
import { maxillariaTenuifolia } from "./maxillaria-tenuifolia.js";
import { colmanaraWildcat } from "./colmanara-wildcat.js";
import { brassavolaPerrini } from "./brassavola-perrini.js";
import { ornithophoraRadicans } from "./ornithophora-radicans.js";
import { coelogyneOvalisVarMiniatum } from "./coelogyne-ovalis-var-miniatum.js";
import { coelogyneCristata } from "./coelogyne-cristata.js";
import { bcTurandotXBlcOrangeShowXBlcChunyeah } from "./bc-turandot-x-blc-orange-show-x-blc-chunyeah.js";
import { cattleyaSchillerianaXCruzeiroDoSul } from "./cattleya-schilleriana-x-cruzeiro-do-sul.js";
import { lcTrickOrTreat } from "./lc-trick-or-treat.js";
import { aciantheraAveniformis } from "./acianthera-aveniformis.js";
import { dryadellaAviceps } from "./dryadella-aviceps.js";
import { speckliniaGrobyi } from "./specklinia-grobyi.js";
import { dryadellaLiliputiana } from "./dryadella-liliputiana.js";
import { leptotesUnicolor } from "./leptotes-unicolor.js";
import { cattleyaAclandiaeTipo } from "./cattleya-aclandiae-tipo.js";
import { cattleyaLueddemannianaTipo } from "./cattleya-lueddemanniana-tipo.js";
import { cattleyaWalkeriana } from "./cattleya-walkeriana.js?v=20260806-1604-auditoria-estrutural";
import { cattleyaIntermedia } from "./cattleya-intermedia.js";
import { cattleyaLeopoldiiDarkPrincess } from "./cattleya-leopoldii-dark-princess.js";
import { laeliaFournieri } from "./laelia-fournieri.js";
import { lockhartiaLunifera } from "./lockhartia-lunifera.js";
import { jacquiniellaGlobosa } from "./jacquiniella-globosa.js";
import { camaridiumCarinatum } from "./camaridium-carinatum.js";
import { aciantheraPubescens } from "./acianthera-pubescens.js";
import { maxillariaLeucaimata } from "./maxillaria-leucaimata.js";
import { coelogyneAsperata } from "./coelogyne-asperata.js";
import { coelogyneFlaccida } from "./coelogyne-flaccida.js";
import { howearaLavaBurst } from "./howeara-lava-burst.js";
import { epicattleyaReneMarques } from "./epicattleya-rene-marques.js";
import { coelogyneFimbriata } from "./coelogyne-fimbriata.js";
import { maxillariaPicta } from "./maxillaria-picta.js";
import { maxillariaChrysantha } from "./maxillaria-chrysantha.js";
import { miltoniaRegnellii } from "./miltonia-regnellii.js";
import { neobenthamiaGracilis } from "./neobenthamia-gracilis.js";
import { aspasiaLunata } from "./aspasia-lunata.js";
import { cyrtopodiumSaintlegerianum } from "./cyrtopodium-saintlegerianum.js";
import { cattleyaWarneri } from "./cattleya-warneri.js?v=20260806-1604-auditoria-estrutural";
import { bulbophyllumElassonotum } from "./bulbophyllum-elassonotum.js";
import { bulbophyllumMiniatum } from "./bulbophyllum-miniatum.js";
import { bulbophyllumSikkimense } from "./bulbophyllum-sikkimense.js";
import { cattliantheCttJewelBox } from "./cattlianthe-ctt-jewel-box.js?v=20260806-1604-auditoria-estrutural";
import { cattleyaLoddigesii } from "./cattleya-loddigesii.js";
import { laeliaCinnabarina } from "./laelia-cinnabarina.js";
import { laeliaLundii } from "./laelia-lundii.js";
import { catasetumMattosianum } from "./catasetum-mattosianum.js";
import { laeliaGloriosa } from "./laelia-gloriosa.js";
import { encycliaBracteata } from "./encyclia-bracteata.js";
import { stanhopeaAssidensis } from "./stanhopea-assidensis.js";
import { dichaeaPendula } from "./dichaea-pendula.js";
import { encycliaGallopavina } from "./encyclia-gallopavina.js";
import { stanhopeaGuttulata } from "./stanhopea-guttulata.js";
import { isabeliaVirginalis } from "./isabelia-virginalis.js";
import { huntleyaMeleagris } from "./huntleya-meleagris.js";
import { miltoniaSpectabilisVarMoreliana } from "./miltonia-spectabilis-var-moreliana.js";
import { oncidiumPubes } from "./oncidium-pubes.js";
import { miltoniaCandida } from "./miltonia-candida.js";
import { ornithocephalusMyrticola } from "./ornithocephalus-myrticola.js";
import { notyliaPubescens } from "./notylia-pubescens.js";
import { trichocentrumFuscum } from "./trichocentrum-fuscum.js";
import { bifrenariaHarrisoniae } from "./bifrenaria-harrisoniae.js";
import { maxillariaPumilaExMinuta } from "./maxillaria-pumila-ex-minuta.js";
import { ludisiaDiscolor } from "./ludisia-discolor.js";
import { bulbophyllumAmbrosia } from "./bulbophyllum-ambrosia.js";
import { cattleyaGuttataVarCoerulea } from "./cattleya-guttata-var-coerulea.js";
import { octomeriaDiaphana } from "./octomeria-diaphana.js";
import { aciantheraBragae } from "./acianthera-bragae.js";
import { blcChiaLin } from "./blc-chia-lin.js";
import { octomeriaMicrantha } from "./octomeria-micrantha.js";
import { gongoraBufonia } from "./gongora-bufonia.js";
import { pabstiellaTrifida } from "./pabstiella-trifida.js";
import { laeliaLundiiAlba } from "./laelia-lundii-alba.js";
import { cattleyaChocolateDrop } from "./cattleya-chocolate-drop.js?v=20260806-1604-auditoria-estrutural";
import { cattleyaABrazilianDream } from "./cattleya-a-brazilian-dream.js";
import { cattleyaViolaceaFSuavissimaXSelf } from "./cattleya-violacea-f-suavissima-x-self.js";
import { cattleyaViolaceaFFlamea } from "./cattleya-violacea-f-flamea.js";
import { cattleyaNobiliorVarAmaliae } from "./cattleya-nobilior-var-amaliae.js";
import { cattleyaWalkerianaFAlbaGnomusXFAlbaSuprema } from "./cattleya-walkeriana-f-alba-gnomus-x-f-alba-suprema.js";
import { zygopetalumMaculatum } from "./zygopetalum-maculatum.js";
import { phalaenopsisMannii } from "./phalaenopsis-mannii.js";
import { bulbophyllumFalcatum } from "./bulbophyllum-falcatum.js";
import { bulbophyllumBuffo } from "./bulbophyllum-buffo.js";
import { bulbophyllumSyciobulbon } from "./bulbophyllum-syciobulbon.js";
import { laeliaCaulencens } from "./laelia-caulencens.js";
import { rodricidium } from "./rodricidium.js";
import { laeliaPraestans } from "./laelia-praestans.js";
import { phaiusTankervilleaeAlba } from "./phaius-tankervilleae-alba.js";
import { stanhopeaHernandezii } from "./stanhopea-hernandezii.js";
import { encycliaOncidioides } from "./encyclia-oncidioides.js";
import { pleurothallisModestissima } from "./pleurothallis-modestissima.js";
import { maxillariaBrasiliensis } from "./maxillaria-brasiliensis.js";
import { isochilusLinearis } from "./isochilus-linearis.js";

export const orquideas = [
    cattleyaPumpkinFestival,
    dendrobiumPolysema,
    bifrenariaVerboonenii,
    bifrenariaAureofulva,
    sophronitisWittigiana,
    sophronitisCernua,
    sophronitisCernuaVarLitoranea,
    zygolumLouisendorf,
    dendrobiumLoddigesii,
    maxillariaSchunkeana,
    mediocalcarDecoratum,
    cattleyaSchilleriana,
    bulbophyllumRothschildianum,
    encycliaMineirinha,
    arpophyllumGiganteum,
    oncidiumTwinkle,
    dendrobiumTenellum,
    dendrobiumAggregatum,
    dendrobiumMoscatum,
    rodrigueziaVenusta,
    rodrigueziaLanceolata,
    oncidiumPumilum,
    eriaHyacinthoides,
    renantheraCoccinea,
    spathoglottisPlicata,
    cyrtopodiumPodophyllum,
    aciantheraGlumacea,
    miltoniaClowesii,
    gomesaRecurva,
    maxillariaSubulata,
    maxillariaTenuifolia,
    colmanaraWildcat,
    brassavolaPerrini,
    ornithophoraRadicans,
    coelogyneOvalisVarMiniatum,
    coelogyneCristata,
    bcTurandotXBlcOrangeShowXBlcChunyeah,
    cattleyaSchillerianaXCruzeiroDoSul,
    lcTrickOrTreat,
    aciantheraAveniformis,
    dryadellaAviceps,
    speckliniaGrobyi,
    dryadellaLiliputiana,
    leptotesUnicolor,
    cattleyaAclandiaeTipo,
    cattleyaLueddemannianaTipo,
    cattleyaWalkeriana,
    cattleyaIntermedia,
    cattleyaLeopoldiiDarkPrincess,
    laeliaFournieri,
    lockhartiaLunifera,
    jacquiniellaGlobosa,
    camaridiumCarinatum,
    aciantheraPubescens,
    maxillariaLeucaimata,
    coelogyneAsperata,
    coelogyneFlaccida,
    howearaLavaBurst,
    epicattleyaReneMarques,
    coelogyneFimbriata,
    maxillariaPicta,
    maxillariaChrysantha,
    miltoniaRegnellii,
    neobenthamiaGracilis,
    aspasiaLunata,
    cyrtopodiumSaintlegerianum,
    cattleyaWarneri,
    bulbophyllumElassonotum,
    bulbophyllumMiniatum,
    bulbophyllumSikkimense,
    cattliantheCttJewelBox,
    cattleyaLoddigesii,
    laeliaCinnabarina,
    laeliaLundii,
    catasetumMattosianum,
    laeliaGloriosa,
    encycliaBracteata,
    stanhopeaAssidensis,
    dichaeaPendula,
    encycliaGallopavina,
    stanhopeaGuttulata,
    isabeliaVirginalis,
    huntleyaMeleagris,
    miltoniaSpectabilisVarMoreliana,
    oncidiumPubes,
    miltoniaCandida,
    ornithocephalusMyrticola,
    notyliaPubescens,
    trichocentrumFuscum,
    bifrenariaHarrisoniae,
    maxillariaPumilaExMinuta,
    ludisiaDiscolor,
    bulbophyllumAmbrosia,
    cattleyaGuttataVarCoerulea,
    octomeriaDiaphana,
    aciantheraBragae,
    blcChiaLin,
    octomeriaMicrantha,
    gongoraBufonia,
    pabstiellaTrifida,
    laeliaLundiiAlba,
    cattleyaChocolateDrop,
    cattleyaABrazilianDream,
    cattleyaViolaceaFSuavissimaXSelf,
    cattleyaViolaceaFFlamea,
    cattleyaNobiliorVarAmaliae,
    cattleyaWalkerianaFAlbaGnomusXFAlbaSuprema,
    zygopetalumMaculatum,
    phalaenopsisMannii,
    bulbophyllumFalcatum,
    bulbophyllumBuffo,
    bulbophyllumSyciobulbon,
    laeliaCaulencens,
    rodricidium,
    laeliaPraestans,
    phaiusTankervilleaeAlba,
    stanhopeaHernandezii,
    encycliaOncidioides,
    pleurothallisModestissima,
    maxillariaBrasiliensis,
    isochilusLinearis
];
