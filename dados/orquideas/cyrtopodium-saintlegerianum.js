// Refinamento final — preservação do conteúdo útil + confronto POWO/WFO/GBIF/IOSPE e consolidação ecológica.

export const cyrtopodiumSaintlegerianum = {
    "id": "cyrtopodium-saintlegerianum",
    "nome": "Cyrtopodium saintlegerianum",
    "genero": "Cyrtopodium",
    "sinonimo": "Cyrtopodium punctatum var. saintlegerianum",
    "tipo": "Espécie botânica",
    "dificuldade": "Moderada",
    "caracteristicas": [
        "Cyrtopodium robusto de regiões sazonais",
        "Frequentemente observado sobre palmeiras no Brasil central",
        "Pseudobulbos longos e caducifólios",
        "Folhas rígidas e estreitas",
        "Flores de cerca de 4 cm",
        "Inflorescência ramificada e muito florífera",
        "Floração no fim do inverno e início da primavera",
        "Prefere muita luz e forte ventilação"
    ],
    "fotos": [
        "imagens/Cyrtopodium saintlegerianum/Cyrtopodium saintlegerianum1.webp",
        "imagens/Cyrtopodium saintlegerianum/Cyrtopodium saintlegerianum2.jpeg",
        "imagens/Cyrtopodium saintlegerianum/Cyrtopodium saintlegerianum3.webp",
        "imagens/Cyrtopodium saintlegerianum/Cyrtopodium saintlegerianum4.jpeg"
    ],
    "descricao": "Uma gigante dos ambientes sazonais sul-americanos! Cyrtopodium saintlegerianum forma pseudobulbos longos, perde as folhas no período de repouso e pode produzir inflorescências muito ramificadas cobertas por numerosas flores amarelas e castanhas de cerca de 4 cm. No Brasil central é especialmente conhecido por crescer sobre palmeiras e troncos expostos, onde recebe muita luz, chuva durante a fase de crescimento e secagem rápida na estação seca.",
    "origem": "América do Sul.",
    "regiao": "Brasil, Paraguai e norte da Argentina.",
    "habitat": "Áreas sazonais · palmeiras/troncos · muita luz.",
    "clima": "Quente sazonal · inverno mais seco.",
    "climaFloracao": "POWO registra a espécie em bioma tropical sazonalmente seco, enquanto observações de campo e IOSPE destacam seu comportamento epífito sobre palmeiras no Brasil central. Em cultivo, essas informações convergem para o mesmo manejo: crescimento quente e bem irrigado quando brotos e folhas estão ativos, seguido de redução hídrica após a maturação. No litoral quente, noites frias intensas não são requisito; luz forte, ventilação e uma estação de repouso perceptível são muito mais importantes.",
    "iluminacao": {
        "sombrite": "30% a 50%",
        "solDireto": "Sim, após aclimatação",
        "horario": "Manhã e parte da tarde",
        "observacoes": "Exige luminosidade muito alta. A exposição sobre palmeiras mostra que a espécie suporta ambiente bastante claro, mas plantas de viveiro devem ser aclimatadas progressivamente. Pouca luz produz pseudobulbos fracos e menor floração; ventilação é indispensável sob sol e calor."
    },
    "floracao": "Mais característica do fim do inverno ao início da primavera. A inflorescência é ereta, paniculada, muito ramificada e pode carregar grande número de flores. Cada flor mede aproximadamente 4 cm. A haste aparece associada a pseudobulbos maduros e reservas acumuladas durante a estação anterior.",
    "adubacao": "🌿 Orgânica: bokashi leve apenas no início e meio do crescimento, sem encostar em brotos ou raízes.\n💧 Foliar/mineral: NPK equilibrado em baixa concentração enquanto houver folhas e raízes ativas.\n🧪 Liberação lenta: poucos grânulos no início do ciclo, principalmente em vaso/cesto.\n🍂 Maturação: reduza quando as folhas começarem a envelhecer.\n🛑 Dormência: suspenda a adubação quando a planta estiver desfolhada e sem crescimento ativo.",
    "rega": "Durante brotação e expansão foliar, regue abundantemente e permita secagem rápida entre irrigações. Quando as folhas amarelecerem naturalmente, reduza progressivamente. Na fase desfolhada, mantenha a planta muito mais seca e proteja de chuvas repetidas, sem transformar a regra em seca absoluta se os pseudobulbos estiverem desidratando de forma extrema.",
    "suporte": "🥇 Tronco grosso ou placa extremamente robusta — aproxima o hábito observado sobre palmeiras.\n🥈 Cesto grande, rígido e muito vazado.\n🥉 Vaso pesado com drenagem extrema para quem não pode montar a planta.\nPlaneje o suporte para longo prazo: raízes aderidas não devem ser arrancadas em replantios frequentes.",
    "substrato": "Montagem: praticamente sem substrato, usando apenas pequena fibra ou esfagno na fixação inicial.\nCesto/vaso: casca grossa + carvão grosso + pedra-pomes ou brita.\nEvite grande volume de material fino ou esfagno compacto, especialmente porque o repouso sazonal exige rápida secagem.",
    "errosComuns": "Pouca luz; regar intensamente após a queda das folhas; manter adubação na dormência; suporte frágil; cortar raízes aderidas; excesso de esfagno; vaso com pouca drenagem; tentar manter crescimento contínuo; confundir queda natural das folhas com doença.",
    "dica": "Se for montar a planta, escolha o suporte como se ele fosse definitivo. Cyrtopodium saintlegerianum pode tornar-se grande e pesado, e raízes aderidas sofrem muito quando são removidas. Durante o crescimento, água e alimento são abundantes; depois da maturação e queda das folhas, faça a transição para um período mais seco, claro e ventilado.",
    "adaptacaoRegional": {
        "litoralQuente": "O calor é compatível com a espécie. Mantenha-a em posição alta, muito clara e ventilada. O principal desafio no litoral é a chuva de inverno: uma cobertura ajuda a criar a redução hídrica sem depender de frio intenso.",
        "montanhaFrio": "Noites mais frescas são bem toleradas e podem reforçar a sazonalidade. Proteja de geadas e, sobretudo, de frio associado a raízes constantemente molhadas."
    },
    "mesesFloracao": [
        8,
        9,
        10
    ],
    "avaliacoes": {
        "cultivo": 3,
        "floracao": 5,
        "perfume": 2,
        "luminosidade": 5,
        "agua": 3,
        "raridade": 4
    }
};
